...
| `DictationStyle` / `RewriteStyle` | preset or the user's own | Both carry a `custom` case whose clause is a setting rather than a file, and both substitute it into the same host block a preset uses — so the framing and the preservation rules cover user-authored text too. `spoken` and `verbatim` send nothing, which is what keeps the default request the measured one. |
