...
///
/// Mirrored word for word in `windows/DoNotType.Core/ModelIdentifier.cs` and
/// `android/app/src/main/kotlin/app/donottype/core/ModelIdentifier.kt`, with the tests duplicated
/// in each language for the reason given in `FailureAdviceTests` — the same mistake should read
/// the same way on all four clients.
public enum ModelIdentifier {
    ...
    public static func isValid(_ value: String) -> Bool { validationMessage(for: value) == nil }

    /// The sentence to put under the field, or nil when there is nothing wrong with what is in it.
    ///
    ...
    public static func validationMessage(for value: String) -> String? {
        ...
        guard let offender = trimmed.first(where: { !isAllowed($0) }) else { return nil }
    ...
    /// ASCII letters and digits only. `Character.isLetter` alone would accept 模 and é, which is
