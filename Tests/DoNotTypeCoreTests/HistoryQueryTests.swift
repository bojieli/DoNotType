...
final class FailureAdviceTests: XCTestCase {
    ...
    /// A gateway answers in lower case and this leads the message, so it is capitalised on the
    /// way past — its words are what has to survive, not its typography.
    func testAShortGatewayMessageIsKept() {
        let advice = FailureAdvice.describe(
            ProviderError.http(status: 503, body: "upstream connect error before headers"))
        XCTAssertTrue(
            advice.message.lowercased().contains("upstream connect error"), advice.message)
    }
