...
final class PipelineIntegrationTests: XCTestCase {
    ...
    func testPreUploadedAudioMatchesInlineAudio() async throws {
        ...
        guard case .preUploaded = plan.route else {
            throw XCTSkip("pre-upload unavailable in this environment")
        }

        let service = try Harness.service()
        let viaUpload = try await service.transcribe(
            audio: audio, context: nil, audioPart: plan.part)
        let viaInline = try await service.transcribe(audio: audio, context: nil)

        XCTAssertEqual(
            TranscriptDiff.normalize(viaUpload.transcript.transcript),
            TranscriptDiff.normalize(viaInline.transcript.transcript),
            "the pre-upload route must produce the same transcript as the inline route")
        XCTAssertGreaterThan(try XCTUnwrap(viaUpload.usage.audioTokens), 0)
    }

    /// The point of the fallback: a broken upload path costs latency, never words.
    func testBrokenUploadSessionFallsBackToInlineRatherThanFailing() async throws {
        try Harness.requireIntegration()
        let audio = try Harness.realAudio("real-talk-gemini15.wav")

        // An unroutable host guarantees the session handshake fails.
        ...
        XCTAssertEqual(plan.route, .inline, "a dead upload service must degrade, not throw")
        let result = try await Harness.service().transcribe(
            audio: audio, context: nil, audioPart: plan.part)
        XCTAssertFalse(result.transcript.transcript.trimmed.isEmpty)
    }

    /// The one case with no way through: too big to inline, and no upload service.
    func testOversizedAudioWithNoUploadServiceReportsRatherThanTruncating() async throws {
        try Harness.requireIntegration()

        let uploader = AudioUploader(
            apiKey: try Harness.apiKey(), baseURL: URL(string: "https://invalid.invalid")!)
        ...
        do {
            ...
            XCTFail("expected tooLargeForInline")
        } catch AudioUploader.UploadError.tooLargeForInline {
            // Correct: surfaced so the caller can store the audio and retry later.
        }
    }
